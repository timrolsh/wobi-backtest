...
void WobiSignalStrategy::OnDepth(const MarketDepthEventMsg& msg) {
    ...
    if (m_debug_on) {
        ...
        const int bid_levels = book.NumBidLevels();
        const int ask_levels = book.NumAskLevels();
        const int symmetric_levels = std::min({m_num_levels, bid_levels, ask_levels});

        cout << "[DEPTH] (" << msg.adapter_time() << ") " << inst.symbol()
             << " | BidLvls=" << bid_levels
             << " | AskLvls=" << ask_levels
             << " | Using=" << symmetric_levels;
